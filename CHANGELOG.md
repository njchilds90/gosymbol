...
All notable changes to gosymbol are documented here.
...
### Added
- `.gitignore` for local build/test artifacts and coverage output
...
### Changed
- `DiffN()` now panics on negative derivative order instead of silently returning the original expression
- README now includes a repository layout section documenting core paths
- CONTRIBUTING checklist now explicitly includes `go vet ./...`
- Package renamed from `sympy` to `gosymbol` for import clarity
